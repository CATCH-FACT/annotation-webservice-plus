...
class SolrplusAnnotatorOLD(AbstractAnnotator):
    def __init__(self, solr_url, rows):
        super(SolrplusAnnotatorOLD, self).__init__()
    ...
    """
    def get_similar(self, tokens, andnot):
        ...
        if len(tokens) > 0:
            r = requests.post(
                ...
                {
                    'wt': 'json',
                    'q': self.buildquery(tokens, andnot),
    ...
    def process(self, doc):
        ...
        if len(tokens) == 0:
            ...
            return self.processdocuments(self.get_similar(tokens, modelid), tokens)
